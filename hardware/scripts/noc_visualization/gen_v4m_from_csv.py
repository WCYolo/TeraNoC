...
def parse_trace_csv(path, needed, strict=False):
    rows = []
    skipped = []
    ...
    with open(path, newline="") as f:
        ...
        for r in cr:
            missing = [k for k in needed if r.get(k) in (None, "")]
            if missing:
                msg = (
                    f"line {cr.line_num}: missing {', '.join(missing)} "
                    f"in row {dict(r)}"
                )
                if strict:
    ...
    if skipped:
        print(f"warning: skipped {len(skipped)} malformed CSV row(s)")
...
def main():
    ...
    rows_raw = parse_trace_csv(args.csv, needed, strict=args.strict_csv)
    if not rows_raw:
